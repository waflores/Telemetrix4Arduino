...
class TestDefaultMasterProfiles(TestCase):
    ...
    def test_clock_low(self):
        def test(
            config: TeensyConfig, spec: I2CSpecification, design: I2CSpecification
        ):
    ...
    def test_data_setup_time(self):
        def test(
            ...
            falling_worst_case = config.data_setup(
                master=True, falling=False
            ).worst_case
            # Meets I2C Specification
    ...
    def test_start_setup_time(self):
        def test(
            ...
            self.assert_in_range(
                config.setup_repeated_start().worst_case, spec.start_setup_time
            )
            # Meets design requirement
            self.assert_in_range(
                config.setup_repeated_start().worst_case, design.start_setup_time
            )
    ...
    def test_log_nominal_times(self):
        ...
        def test(
            ...
            print_parameter("START Hold Time (tHD:STA)", config.start_hold())
            print_parameter(
                "Setup Repeated START (tSU:STA)", config.setup_repeated_start()
            )
