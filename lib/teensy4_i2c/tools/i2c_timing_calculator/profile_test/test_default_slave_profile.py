...
from i2c_timing_calculator.profile_test.i2c_specification import (
    fast_mode_plus_spec,
    I2CSpecification,
    Range,
    standard_mode_spec,
...
class TestDefaultSlaveProfile(TestCase):
    ...
    def test_data_setup_time(self):
        def test(config: TeensyConfig, spec: I2CSpecification):
            ...
            self.assertLessEqual(
                config.clock_low().worst_case, spec.scl_low_time.min * 1.05
            )
            # Meets I2C Specification
            self.assert_in_range(
                config.data_setup(False, True).worst_case, spec.data_setup_time
            )
            self.assert_in_range(
                config.data_setup(False, False).worst_case, spec.data_setup_time
            )
            # BusRecorder will record edges separately
            self.assertGreaterEqual(
                config.data_setup(False, True).worst_case, self.bus_recorder_resolution
            )
            self.assertGreaterEqual(
                config.data_setup(False, False).worst_case, self.bus_recorder_resolution
            )
    ...
    def test_log_nominal_times(self):
        ...
        def test(config: TeensyConfig, spec: I2CSpecification):
            print(config.name)
            print_parameter("Clock Low Time (tLOW)", config.clock_low())
            print_parameter("Clock High Time (tHIGH)", config.clock_high())
            print_parameter(
                "Data Hold Time (tHD:DAT) - Slave 1->0",
                config.data_hold(master=False, falling=True),
            )
            # print_parameter(f"Data Valid Time (tVD:DAT) - Slave 1->0", config.data_valid(master=False, falling=True))
            print_parameter(
                "Data Setup Time (tSU:DAT) - Slave 1->0",
                config.data_setup(master=False, falling=True),
            )
            print(
                f"Glitch filters. SDA: {config.sda_glitch_filter():.0f} nanos. SCL: {config.scl_glitch_filter():.0f} nanos"
            )
            print()
