...
class TeensyConfig:
    def __init__(
        ...
        self.SCL_RISETIME = (
            scl_risetime * time_to_rise_to_teensy_trigger_voltage
        ) / self.period
        self.SDA_RISETIME = (
            sda_risetime * time_to_rise_to_teensy_trigger_voltage
        ) / self.period
        self.DATAVD = datavd
    ...
    def data_hold(self, master: bool, falling: bool):
        adjustment = (
            (self.fall_time * time_to_fall_to_0_7_vdd)
            if falling
            else (self.sda_risetime * time_to_rise_to_0_3_vdd)
        )
        if master:
            nominal = self.scale * (self.DATAVD + 1)
            i2c_value = (
                nominal - (self.fall_time * time_to_fall_to_0_3_vdd) + adjustment
            )
            worst_case_adjustment = (
                (self.fall_time * time_to_fall_to_0_7_vdd)
                if falling
                else (self.max_rise * time_to_rise_to_0_3_vdd)
            )
            worst_case = (
                nominal
                - (self.fall_time * time_to_fall_to_0_3_vdd)
                + worst_case_adjustment
            )
        else:
            ...
            i2c_value = (
                nominal
                - (self.fall_time * time_to_fall_from_trigger_to_0_3_vdd)
                + adjustment
            )
            worst_case = nominal - (
                self.max_fall * time_to_fall_from_trigger_to_0_3_vdd
            )
    ...
    def data_setup(self, master: bool, falling: bool):
        ...
        if falling:
            worst_case = (
                i2c_value
                if master
                else nominal - (self.max_fall * time_to_fall_to_0_3_vdd)
            )
        else:
            worst_case = (
                i2c_value
                - (self.max_rise - self.sda_risetime) * time_to_rise_to_0_7_vdd
            )
        return Parameter(i2c_value, nominal, worst_case)
    ...
    def start_hold(self):
        ...
        i2c_value = (
            nominal
            - (self.fall_time * time_to_rise_to_0_7_vdd)
            + (self.fall_time * time_to_rise_to_0_3_vdd)
        )
    ...
    def setup_repeated_start(self):
        ...
        i2c_value = (
            nominal
            - (self.scl_risetime * time_to_rise_to_0_7_vdd)
            + (self.fall_time * time_to_fall_to_0_7_vdd)
        )
        worst_case_nominal = self.scale * (
            self.SETHOLD + 1 + self.SCL_LATENCY(self.max_rise)
        )
        worst_case = (
            worst_case_nominal
            - (self.max_rise * time_to_rise_to_0_7_vdd)
            + (self.fall_time * time_to_fall_to_0_7_vdd)
        )
        return Parameter(i2c_value, nominal, worst_case)
    ...
    def stop_setup(self):
        ...
        i2c_value = (
            nominal
            - (self.scl_risetime * time_to_rise_to_0_7_vdd)
            + (self.sda_risetime * time_to_rise_to_0_3_vdd)
        )
        worst_case_nominal = self.scale * (
            self.SETHOLD + 1 + self.SCL_LATENCY(self.max_rise)
        )
    ...
    def clock_high(self):
        ...
        worst_case_nominal = self.scale * (
            self.CLKHI + 1 + self.SCL_LATENCY(self.max_rise)
        )
        worst_case = (
            worst_case_nominal
            - (self.max_rise * time_to_rise_to_0_7_vdd)
    ...
    def clock_frequency(self):
        period_nominal = self.scale * (
            self.CLKHI + self.CLKLO + 2 + self.SCL_LATENCY(self.scl_risetime)
        )
        worst_case_period_nominal = self.scale * (
            self.CLKHI + self.CLKLO + 2 + self.SCL_LATENCY(0)
        )
    ...
    def bus_free(self):
        ...
        def get_nominal_and_i2c_value(rise_time: int) -> tuple[int, int]:
            if rise_time > 1000:
                offset = (
                    ((rise_time - 1000) * time_to_rise_to_0_7_vdd) / self.scale
                ) + 1
            else:
                offset = 2
                if self.BUSIDLE > 1:
            ...
            i2c_value = (
                nominal
                - (rise_time * time_to_rise_to_0_7_vdd)
                + (self.fall_time * time_to_fall_to_0_7_vdd)
            )
            return nominal, i2c_value

        nominal, i2c_value = get_nominal_and_i2c_value(self.sda_risetime)
        worst_case_nominal, worst_case_i2c_value = get_nominal_and_i2c_value(
            self.max_rise
        )
